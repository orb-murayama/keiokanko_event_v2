-- ========================================
-- キャンセルポリシーを商品からイベントに移動
-- ========================================

-- Step 1: eventsテーブルにキャンセルポリシーカラムを追加（1つずつ実行）
ALTER TABLE events ADD COLUMN cancel_policy TEXT;

-- Step 2: 既存データのクリーンアップ（商品のキャンセルポリシーをクリア）
-- 注意: 既存の商品データのキャンセルポリシーは削除されます
UPDATE products 
SET 
  cancel_policy = NULL,
...
-- Step 3: productsテーブルからキャンセルポリシーカラムを削除
-- 注意: SQLiteではALTER TABLE DROP COLUMNがサポートされていないため、
-- テーブルの再作成が必要です。本番環境では慎重に実行してください。

-- 一時テーブルを作成（キャンセルポリシーカラムを除く）
