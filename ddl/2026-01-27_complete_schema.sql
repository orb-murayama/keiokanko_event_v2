...
-- 
-- 1. キャンセルポリシー
--    - イベントテーブルに統合（cancellation_days_1~3, cancellation_rate_1~3）
--    - 商品テーブルのキャンセルポリシーは非推奨（レガシー互換のため残存）
--    - SQLiteのカラム数制限（100）により3件に制限
--
-- 2. 静的ファイル化
--    - 商品フォーム設定: /products-form-fields.html?id=xxx
--    - 商品在庫管理: /products-stocks.html?id=xxx
--    - イベント管理: /events-list.html, /events-form.html?id=xxx
--
-- 3. API-First設計
--    - すべての管理画面は静的HTMLファイル
--    - データ取得・更新はREST APIを使用
--    - バンドルサイズ削減による高速化
--
-- 4. 親子フィールド機能
--    - product_form_fields と event_form_fields で条件分岐フィールドをサポート
--    - parent_field_id, parent_condition で親子関係を定義
--    - indent_level でネスト表示レベルを管理
--
-- 5. 共有在庫プール
--    - 複数商品で在庫を共有可能
--    - available_stock は仮想カラム（total_stock - booked）
--
-- ========================================
