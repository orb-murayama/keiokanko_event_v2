...
-- 商品フォーム項目マスター - シードデータ
-- ============================================
-- 目的: 商品フォーム項目のサンプルデータを投入
-- 対象: 宿泊プラン（product_id: 1）、体験ツアー（product_id: 2）
-- ============================================

-- ============================================
-- 宿泊プラン（product_id: 1）のフォーム項目
-- ============================================
...
(1, 'select', 'breakfast_time', '朝食の時間帯', '["7:00-7:30","7:30-8:00","8:00-8:30","8:30-9:00","9:00-9:30"]', 0, 'ご希望の朝食時間帯をお選びください', 60, NULL, (SELECT id FROM product_form_fields WHERE product_id=1 AND field_name='breakfast_option'), '希望する', 1),
(1, 'checkbox', 'breakfast_menu', '朝食メニュー', '["和食","洋食","アレルギー対応食"]', 0, 'ご希望のメニューを選択してください（複数選択可）', 70, NULL, (SELECT id FROM product_form_fields WHERE product_id=1 AND field_name='breakfast_option'), '希望する', 1);
...
-- 経験者向け追加情報
INSERT INTO product_form_fields (product_id, field_type, field_name, field_label, field_options, is_required, description, display_order, parent_field_id, parent_condition, indent_level) VALUES
(2, 'select', 'equipment_rental', '装備レンタル', '["必要","不要（持参）"]', 0, '装備のレンタル希望をお選びください', 50, (SELECT id FROM product_form_fields WHERE product_id=2 AND field_name='experience_level'), '経験者', 1);
...
(2, 'text', 'pickup_location', '送迎場所（住所）', NULL, 0, '送迎希望の場合、お迎え場所を入力してください', 70, (SELECT id FROM product_form_fields WHERE product_id=2 AND field_name='pickup_option'), '希望する', 1, '例: 〇〇ホテル、△△駅など'),
(2, 'time', 'pickup_time', '希望送迎時刻', NULL, 0, 'ご希望の送迎時刻をお選びください', 80, (SELECT id FROM product_form_fields WHERE product_id=2 AND field_name='pickup_option'), '希望する', 1, '集合時刻の30分前が目安です');
...
-- 条件分岐フィールドを確認
-- SELECT 
--   pf1.field_label as parent_field,
--   pf1.field_type as parent_type,
--   pf2.field_label as child_field,
--   pf2.parent_condition as condition_value,
--   pf2.indent_level
-- FROM product_form_fields pf2
-- LEFT JOIN product_form_fields pf1 ON pf2.parent_field_id = pf1.id
-- WHERE pf2.parent_field_id IS NOT NULL
-- ORDER BY pf2.product_id, pf2.display_order;
